/**
 * Script to run migration 081: Shop Entry Requests
 */
...
async function runMigration() {
  console.log('🔧 Running Migration 081: Shop Entry Requests\n');

  const migrationPath = path.join(__dirname, '../src/migrations/081_add_shop_entry_requests.sql');
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        let result = await supabase.rpc('query', { sql_text: statement });
    ...
    console.log('2. Open file: src/migrations/081_add_shop_entry_requests.sql');
