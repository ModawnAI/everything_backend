...
interface NotificationSeed {
  user_id: string;
  notification_type: 'reservation_confirmed' | 'reservation_cancelled' | 'point_earned' | 'referral_success' | 'system';
  title: string;
  message: string;
  status: 'unread' | 'read';
  related_id?: string;
...
async function seedNotifications() {
  try {
    ...
    logger.info(`Using test user: ${testUserId}`);
    ...
    logger.info(`✅ Successfully seeded ${insertedNotifications?.length || 0} notifications`);
    ...
    if (settingsError) {
      logger.warn('Failed to create notification settings:', settingsError);
    } else {
      logger.info('✅ Created notification settings for user');
    }
