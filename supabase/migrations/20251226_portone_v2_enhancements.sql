-- =============================================
-- Migration: PortOne V2 Enhancements
-- Date: 2025-12-26
...
CREATE TABLE IF NOT EXISTS public.webhook_logs (
    ...
    webhook_type VARCHAR(100) NOT NULL, -- Transaction.Paid, Transaction.Cancelled, etc.
...
CREATE POLICY billing_keys_admin_all ON public.billing_keys
    FOR ALL
    USING (
        auth.uid() IN (SELECT user_id FROM public.users WHERE role IN ('superadmin', 'admin'))
    );
