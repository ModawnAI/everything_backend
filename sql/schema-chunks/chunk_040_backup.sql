...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'shop_categories') THEN
        CREATE POLICY IF NOT EXISTS "Allow public read access to active shop categories" ON public.shop_categories
            FOR SELECT USING (is_active = TRUE);

        CREATE POLICY IF NOT EXISTS "Allow authenticated users to read all shop categories" ON public.shop_categories
            FOR SELECT USING (auth.role() = 'authenticated');

        CREATE POLICY IF NOT EXISTS "Allow admin full access to shop categories" ON public.shop_categories
