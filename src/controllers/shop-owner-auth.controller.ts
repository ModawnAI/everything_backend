...
export class ShopOwnerAuthController {
  ...
  async refreshSession(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.info('[REFRESH] Attempting to refresh shop owner session', {
        ipAddress,
      ...
      logger.info('[REFRESH] ✅ Shop owner session refreshed successfully', {
        shopOwnerId: result.shopOwner.id,
        email: result.shopOwner.email,
        shopId: result.shopOwner.shop.id,
      ...
      logger.error('[REFRESH] ❌ Shop owner session refresh failed', {
        error: errorMessage,
        stack: error instanceof Error ? error.stack : undefined,
        ipAddress: req.ip
      });
